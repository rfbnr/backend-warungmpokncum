...
router.get("/orders/:id/receipt", async (req, res) => {
  try {
    ...
    const rows = (order.OrderItems || [])
      .map((it) => {
        const addons = (it.OrderItemAddons || [])
          .map(
            (a) =>
              `<div class="addon">+ ${a.addon_name} (Rp${fmt(
                a.addon_price,
              )})</div>`,
          )
          .join("");
...
router.post("/qris/:sessionId/simulate-paid", async (req, res) => {
  ...
  if (qs.status !== "PENDING")
    return res.status(400).json({ message: `Session already ${qs.status}` });
  ...
  try {
    ...
    const paidAt = new Date();
    await order.update(
      { status: "PAID", payment_method: "QRIS", paid_at: paidAt },
      { transaction: t },
    );
    await Payment.create(
      {
